...
class RegistrationForm(forms.Form):
    username = forms.CharField(
        max_length=255, validators=[validate_slug], label="Имя пользователя",
    )
    password = forms.CharField(
        max_length=255, validators=[validate_slug], label="Пароль",
    )
    email = forms.EmailField(
        label="Электронная почта", help_text="Не обязательное", required=False,
    )
